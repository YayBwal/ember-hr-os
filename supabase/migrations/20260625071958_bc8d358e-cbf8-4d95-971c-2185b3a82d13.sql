...
DO $$ BEGIN
  CREATE TYPE public.employment_type AS ENUM ('remote','on_site');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

ALTER TABLE public.employees
  ADD COLUMN IF NOT EXISTS employment_type public.employment_type NOT NULL DEFAULT 'on_site';
...
BEGIN
  RETURN QUERY
  WITH t AS (
    SELECT assignee_employee_id AS eid,
           COUNT(*) FILTER (WHERE status IN ('todo','in_progress','done')) AS total_tasks,
           COUNT(*) FILTER (WHERE status='done') AS done_tasks
    FROM public.tasks
    WHERE org_id = v_org
      AND assignee_employee_id IS NOT NULL
      AND (due_date BETWEEN v_start AND v_end
           OR (completed_at >= v_start AND completed_at < v_start + interval '1 month'))
    GROUP BY assignee_employee_id
  ),
  a AS (
    SELECT employee_id AS eid,
           COUNT(*) FILTER (WHERE status='present') AS pres,
           COUNT(*) FILTER (WHERE status='late') AS lat,
           COUNT(*) FILTER (WHERE status='absent') AS abs_d,
           COUNT(*) AS logged
...
GRANT EXECUTE ON FUNCTION public.compute_kpi_dashboard(date) TO authenticated;
